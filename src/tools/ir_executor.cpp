...
bool ir::Executor::exec_ir(size_t n) {
    while (n--) {
        ...
        case Operator::store: {
            ...
            if ((inst->des.type == Type::Int || inst->des.type == Type::IntLiteral) && inst->op1.type == Type::IntPtr) {
                find_src_operand(inst->op1)._val.iptr[off] = find_src_operand(inst->des)._val.ival;
            } else if ((inst->des.type == Type::Float || inst->des.type == Type::FloatLiteral) && inst->op1.type == Type::FloatPtr) {
